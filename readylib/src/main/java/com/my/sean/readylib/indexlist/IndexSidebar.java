package com.my.sean.readylib.indexlist;
...
public class IndexSidebar extends View
{
	...
	private void init()
	{
		...
		sections = new String[] { st, "A", "B", "C", "D", "E", "F", "G", "H",
	...
	private void setHeaderTextAndscroll(MotionEvent event)
	{
		...
		if (sectionIndexter == null) {
			if (adapter instanceof HeaderViewListAdapter) {
				sectionIndexter = (SectionIndexer) ((HeaderViewListAdapter) adapter)
						.getWrappedAdapter();
		...
		try {
			for (int i = adapterSections.length - 1; i > -1; i--) {
				if (adapterSections[i].equals(headerString)) {
					mListView.setSelection(sectionIndexter
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		case MotionEvent.ACTION_DOWN: {
			...
			setBackgroundResource(R.drawable.shape_sidebar_background_pressed);
			return true;
		}
